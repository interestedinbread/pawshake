...
const uploadDocument = async (req, res) => {
    try {
        ...
        const policyId = req.body.policyId?.trim();
        if (!policyId) {
            res.status(400).json({
        ...
        const policyLookup = await db_1.db.query(`SELECT id, name FROM policies WHERE id = $1 AND user_id = $2`, [policyId, userId]);
        ...
        for (const file of files) {
            try {
                ...
                const extractedData = await (0, documentServices_1.extractTextFromPDF)(pdfBuffer);
        ...
        `;
                const insertResult = await db_1.db.query(insertQuery, [
                    userId,
                    policyId,
                    file.originalname,
                    extractedData.text,
                ...
                const chunks = await (0, chunkingService_1.chunkDocument)(extractedData.text, extractedData.pageCount, document.id, policyId);
                await (0, vectorService_1.storeChunks)(chunks, document.id, policyId);
      ...
      `, [policyId, userId]);
        ...
        if (policyDocumentsResult.rows.length > 0) {
            try {
                policySummary = await (0, policiesController_1.extractAndSavePolicySummary)(policyId, userId);
            }
...
const getPolicySummary = async (req, res) => {
    ...
    `;
        const summaryResult = await db_1.db.query(summaryQuery, [policyId, userId]);
        ...
        const documentsResult = await db_1.db.query(`
...
exports.getPolicySummary = getPolicySummary;
const getPolicyDocuments = async (req, res) => {
    try {
        ...
        const policyLookup = await db_1.db.query(`SELECT id, name, description, created_at, updated_at FROM policies WHERE id = $1 AND user_id = $2`, [policyId, userId]);
        if (policyLookup.rows.length === 0) {
            res.status(404).json({
                error: 'Policy not found',
      ...
      ORDER BY created_at ASC
    `;
        const documentsResult = await db_1.db.query(documentsQuery, [policyId, userId]);
        res.status(200).json({
...
exports.getPolicyDocuments = getPolicyDocuments;
const deleteDocument = async (req, res) => {
    try {
        ...
        const documentCheck = await db_1.db.query(`SELECT id, policy_id, filename FROM documents WHERE id = $1 AND user_id = $2`, [documentId, userId]);
        ...
        let deletedChunks = 0;
        try {
            deletedChunks = await (0, vectorService_1.deleteChunksByDocumentId)(documentId);
        }
        catch (chromaError) {
    ...
    `;
        const deleteResult = await db_1.db.query(deleteQuery, [documentId, userId]);
        ...
        if (policyId) {
            try {
                await db_1.db.query('DELETE FROM policy_summaries WHERE policy_id = $1', [policyId]);
            }
            catch (summaryError) {
                // Log error but don't fail the deletion

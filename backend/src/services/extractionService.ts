...
export async function extractPolicySummary(
  ...
  try {
    ...
    let extractedSummary: Partial<PolicySummary>;
    try {
    ...
    throw new Error(
      `Failed to extract policy summary: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
...
async function postProcessAndValidate(
  ...
  if (validated.deductible) {
    const deductible = validated.deductible as DeductibleInfo;
  ...
  if (validated.waitingPeriod) {
    const wp = validated.waitingPeriod as WaitingPeriodInfo;
  ...
  const annualMaxValue = validated.annualMaximum as number | string | null | undefined;
  ...
  const perIncidentMaxValue = validated.perIncidentMaximum as number | string | null | undefined;
